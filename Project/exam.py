...
class Exam:
    def __init__(self, BottomFrame):
        ...
        self.img5 = Image.open("Images/exam.png")  # the  pass of the photo
        self.img5.thumbnail((200, 200))  # To cut the photo
    ...
    def openExamWindow(self):  # A Function Used To open Exam Window
        STDW = ExamWindow()
...
class ExamWindow:
            def __init__(self):
                ...
                self.classe = StringVar()               # to convert the data that the user enter it to string.
                self.professor = StringVar()
                self.dexam = StringVar()
                self.timeExam = StringVar()

                self.GroupName = Entry(self.frameleft, fg='#4F4F4F', font=('tahoma', 12, 'bold'),
                                       textvariable=self.group)
                self.GroupName.place(x=170, y=20, width=200, height=40)
                self.classRoom = Entry(self.frameleft, fg='#4F4F4F', font=('tahoma', 12, 'bold'),
                                       textvariable=self.classe)
                self.classRoom.place(x=170, y=80, width=200, height=40)
                self.Professor = Entry(self.frameleft, fg='#4F4F4F', font=('tahoma', 12, 'bold'),
                                       textvariable=self.professor)
                ...
                self.TimeEntry = ttk.Combobox(self.frameleft,
                                              values=["", "8:00", "9:00", "10:00", "11:00", "12:00", "14:00", "15:00",
                                                      "16:00", "17:00"],
                ...
                self.buttonUpdate = Button(self.frameleft, command=self.update, text="UPDATE",
                                           font=('tahoma', 10, 'bold'))
                self.buttonUpdate.place(x=100, y=700, width=60, height=60)
                self.buttonDelete = Button(self.frameleft, command=self.delete, text="DELETE",
                                           font=('tahoma', 10, 'bold'))
            ...
            def update(self):
                ...
                val = (
                    self.group.get(), self.classe.get(), self.professor.get(), self.DateExam.get_date(),
                    self.timeExam.get(),
                    self.iid)
                mycursor.execute(sql, val)
                mydb.commit()
